...
import {
  ...
  Paperclip, // 중복된 CheckSquare를 구분하기 위해 alias 사용
} from 'lucide-react';
...
import { AvatarStack } from '../../common/AvartarStack'; // 경로는 가정합니다.
...
interface BoardManageModalProps {
  ...
  editData?: {
    ...
    importance: string;
    assigneeId?: string;
    participantIds?: string[]; // 💡 [수정] Participants ID 배열 타입 명시
  } | null;
  workspaceId: string;
  onClose: () => void;
  onBoardCreated: () => void;
...
export const BoardManageModal: React.FC<BoardManageModalProps> = ({
  ...
  // 💡 [수정] 작업자 (다건 선택) 상태: editData의 participantIds로 초기화
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        formData.append('participants', JSON.stringify(selectedParticipantIds)); // 배열을 JSON 문자열로 변환
        formData.append('customFields', JSON.stringify(customFields)); // 객체를 JSON 문자열로 변환
        formData.append('file', selectedFile); // 파일 데이터

        // 💡 백엔드에서 파일과 JSON 데이터를 모두 받을 수 있는 전용 API를 사용해야 함
        if (isEditing) {
          // await updateBoardWithFile(editData!.boardId, formData);
          console.warn('Update with file API call is mocked. Using FormData:', formData);
          // 임시로 일반 업데이트 API 호출 (실제 파일은 무시됨)
          // await updateBoard(editData!.boardId, { /* JSON 데이터만 */ });
        } else {
          // await createBoardWithFile(formData as FormData);
          console.warn('Create with file API call is mocked. Using FormData:', formData);
          // 임시로 일반 생성 API 호출 (실제 파일은 무시됨)
          // await createBoard({ /* JSON 데이터만 */ });
        }
      } else {
  ...
  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[90] modal-container"
      onClick={onClose}
    >
      <div
                ...
                <div className="relative assignee-dropdown-container">
                  ...
                  >
                    <span className="flex items-center gap-2">
                      {currentAssignee ? (
                        <>
                          {/* 💡 아바타는 AvatarStack 컴포넌트가 담당해야 하지만, 여기서는 간단히 이니셜 표시 */}
                          <div
